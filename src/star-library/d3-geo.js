...
function linePoint(lambda, phi) {
  ...
  if (p0) {
    ...
    if (
      antimeridian ^ (sign2 * lambda2 < lambdai && lambdai < sign2 * lambda)
    ) {
...
function polygonContains(polygon, point) {
  ...
  return (
    (angle2 < -epsilon || (angle2 < epsilon && sum < -epsilon2)) ^ (winding & 1)
  );
}
...
function clipRectangle(x02, y02, x12, y12) {
  ...
  function interpolate2(from, to, direction, stream) {
    ...
    if (
      ...
      do stream.point(a === 0 || a === 3 ? x02 : x12, a > 1 ? y12 : y02);
...
function graticule() {
  ...
  function lines() {
    return range$1(ceil(X02 / DX) * DX, X12, DX)
      .map(X)
      .concat(range$1(ceil(Y02 / DY) * DY, Y12, DY).map(Y))
      .concat(
        range$1(ceil(x02 / dx) * dx, x12, dx)
          .filter(function (x2) {
  ...
  function resampleLineTo(
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a = a0 + a1,
        ...
        phi2 = asin((c /= m)),
        lambda22 =
          abs(abs(c) - 1) < epsilon || abs(lambda02 - lambda12) < epsilon
            ? (lambda02 + lambda12) / 2
            : atan2(b, a),
