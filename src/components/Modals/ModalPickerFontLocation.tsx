...
import {setFontLocationAction} from "../../redux/actions/creators/controllerLocationActionCreators";

function ModalPickerFontLocation() {
  // console.log("ModalPickerFontLocation");

  const dispatch = useAppDispatch();

  const navigation = useNavigation();

  const {fonts, currentFont} = useAppSelector(Selectors.getFontsLocation);
  ...
  const onSumbit = useCallback(() => {
    if (fontSelect) {
      dispatch(setFontLocationAction(fontSelect));
    }
...
export default React.memo(ModalPickerFontLocation);
